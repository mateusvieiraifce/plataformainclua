...
class VerifyPaymentSignature
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (isset($user) && $user->tipo_user == "P" && $user->etapa_cadastro == "F") {   
            ...
            //Torna a assinatura obrigatória para acessar todos os serviços.
            if (!$assinatura) {
                session()->flash('msg', ['valor' => trans("Você não possui uma assinatura. Para ter acesso aos serviços da platarforma cadastre seu cartão de crédito."), 'tipo' => 'danger']);

                return redirect()->route('paciente.financeiro');
            }
            
            /* 
            ============== UTILIZADO QUANDO A RENOVAÇÃO ERA REALIZADA NO ATO DO LOGIN ===============
