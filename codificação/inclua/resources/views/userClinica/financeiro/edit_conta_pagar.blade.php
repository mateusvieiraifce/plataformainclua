...
@section('title', 'Edição de dados da conta para pagar')
@section('content')
    <div class="row">
        <div class="col-md-7 mr-auto">
            <div class="card card-register card-white">
                ...
                <div class="card-body">                    
                    <form class="form" method="post" action="{{ route('clinica.financeiro.update', $conta->id) }}">
                        @csrf
                        @method('put')
                        <div class="form-group">
                            ...
                            <div class="input-group {{ $errors->has('descricao') ? ' has-danger' : '' }} input-medium">
                                <input type="text" id="descricao" class="form-control border-full {{ $errors->has('descricao') ? ' is-invalid' : '' }}"
                                    name="descricao" placeholder="Descrição" value="{{ old('descricao') ?? $conta->descricao }}" required>
