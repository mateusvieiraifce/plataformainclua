    ...
    <div class="card">
      ...
      <div class="card-body">
        <form method="post" action="{{route('especialistaclinica.save', $clinica->id)}}">
          ...
          <div class="col-md-12 px-8">
            <div class="form-group">
              ...
              <select name="especialista_id" id="especialista_id" class="form-control" title="Por favor selecionar ..."
                required style="border-color: white">
                @foreach($especialistas as $iten)
          <option style="color: #2d3748" value="{{old('especialista_id', $iten->id)}}"
        ...
        @endforeach
          ...
          <a href="{{route('especialistaclinica.list', $clinica->id)}}" class="btn btn-info"><i
              class="tim-icons icon-email-85"></i><span> Enviar convite</span></a>
      </div>
      </form>
    </div>
  </div>
