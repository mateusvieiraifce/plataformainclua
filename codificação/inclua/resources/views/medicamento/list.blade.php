      ...
      <div class="col-lg-12 col-md-12">
         <div class="card card-tasks">
            <div class="card-header">
               <h6 class="title d-inline">Lista de medicamento </h6>
               <div class="dropdown">
                  <button type="button" class="btn btn-link dropdown-toggle btn-icon" data-toggle="dropdown">
                     <i class="tim-icons icon-settings-gear-63"></i>
                  </button>
                  <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink">
                     <a class="dropdown-item" href="{{route('medicamento.new')}}">Adicionar</a>
                  </div>
               </div>
            </div>
            <div class="card-body">
               <div class="table-responsive">
                  <table class="table">
                   ...
                   <tr>
                     <td>{{$ent->nome_comercial}}</td>

                     <td>{{$ent->nome_generico}}</td>
                     <td>
                       <a rel="tooltip" title="Editar" class="btn btn-link" data-original-title="Edit"
                        href="{{route('medicamento.edit', $ent->id)}}">
                        <i class="tim-icons icon-pencil"></i>
                       </a>
                     </td>
                     <td>
                       <a href="{{route('medicamento.delete', $ent->id)}}"
               ...
               @endforeach 
                  ...
                  <div>
                     @if ($lista->lastPage() > 1)
                                  @php
                            $paginator = $lista;
                            $paginator->url = route('medicamento.list');
                        @endphp
                                  <ul class="pagination">
