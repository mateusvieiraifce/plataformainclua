     ...
     <div class="col-lg-12 col-md-12">
         <div class="card card-tasks">
            <div class="card-header">
                  <h6 class="title d-inline">Lista de especialidade </h6>
<div class="dropdown">
<button type="button" class="btn btn-link dropdown-toggle btn-icon" data-toggle="dropdown">
   <i class="tim-icons icon-settings-gear-63"></i>
 </button>
 <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink">
     <a class="dropdown-item" href="{{route('especialidade.new')}}">Adicionar</a>
 </div>
</div>
            </div>
            <div class="card-body">
...
<table class="table"> 
  ...
  @if(sizeof($lista)>0)
    @foreach($lista as $ent)
      <tr>
        ...
        <td> 
         <a href="{{route('especialidade.delete',$ent->id)}}" onclick="return confirm('Deseja relamente excluir?')"  rel="tooltip"
 ...
 <div>
                  @if ($lista->lastPage() > 1)
                     @php
                     $paginator=$lista;
                     $paginator->url = route('especialidade.list');
                  @endphp
                  <ul class="pagination">
