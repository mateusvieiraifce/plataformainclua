...
@section('content')
            ...
            <div class="col-lg-12 col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <form action="{{route('consulta.listConsultaPorEspecialistaPesquisar')}}" method="post"
                              id="pesquisar">
                            @csrf
                                ...
                                <div class="col-md-2 px-8">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="inicio_data" id="inicio_data"
                                               class="form-control"
                                               value="{{ (isset($inicio_data)) ? $inicio_data : date('Y-m-d') }}">
                                    </div>
                                </div>
                                <div class="col-md-2 px-8">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="final_data" id="final_data"
                                               class="form-control"
                                               value="{{ (isset($final_data)) ? $final_data : date('Y-m-d') }}">
                                    </div>
                                </div>
                                <div class="col-md-3 px-8">
                                    <div class="form-group">
                                        ...
                                        <input type="text" placeholder="Nome do paciente" name="nomepaciente"
                                               id="nomepaciente" class="form-control"
                                               value="{{ isset($nomepaciente) ? $nomepaciente : null}}">
                                    </div>
                                </div>
                                <div class="col-md-2 px-8">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="status" name="status" required>
                                            ...
                                            <option value="Sala de espera"
                                                    @if($status == "Sala de espera") selected @endif>Sala de espera
                                            </option>
                                            <option value="Aguardando atendimento"
                                                    @if($status == "Aguardando atendimento") selected @endif>Aguardando
                                                atendimento
                                            </option>
                                            <option alue="Cancelada" @if($status == "Cancelada") selected @endif>
                                                Cancelada
                                            </option>
                                            <option alue="Em atendimento"
                                                    @if($status == "Em atendimento") selected @endif>Em atendimento
                                            </option>
                                            <option value="Finalizada" @if($status == "Finalizada") selected @endif>
                                                Finalizada
                                            </option>
                                        </select>
                                    </div>
                                        ...
                                        <div class="input-button-inline">
                                            <select name="clinica_id" id="clinica_id" class="form-control">
                                                @foreach($clinicas as $iten)
                                                    <option value="{{old('especialidade_id', $iten->id)}}"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if (!$consulta->id_usuario_cancelou)
                                                    @if(!$consulta->isPago)
                                                        <a href="#" rel="tooltip" title="Efetuar Pagamento"
                                                           class="btn btn-default button-small-table"
                                                           data-target="#modal-form-pagar-consulta"
                                                           data-toggle="modal"
                                                           onclick="setModalPagamentoConsulta('{{ $consulta->id }}', '{{ number_format($consulta->preco, 2, ',', '.') }}')">
                                                            Efetuar pagamento
                                                        </a>
                                                    @else
                                                        <button type="button"
                                                                class="btn btn-success button-small-table consulta-paga">
                                                            Consulta paga
                                                        </button>
                                                    @endif
                                                @endif
                                                ...
                                                @if (!$consulta->id_usuario_cancelou && !($consulta->status == "Finalizada"))
                                                    <a rel="tooltip" title="Iniciar atendimento"
                                                       class="btn btn-primary button-small-table"
                                                       data-original-title="Edit"
                                                       href="{{route('especialista.iniciarAtendimento', [$consulta->id, "prontuarioatual"])}}">
                                                        Iniciar atendimento
                                                    </a>
                                                @else
                                                ...
                                                <br>
                                                <a rel="tooltip" title="Cancelar"
                                                   class="btn btn-default button-small-table" data-original-title="Edit"
                                                   href="{{route('paciente.prontuario',['id_paciente'=>$consulta->paciente_id])}}">
                                                    Prontuário
                                                </a>

                                                <br>
                                                <a class="btn btn-default button-small-table" href="#" target="_blank"
                                                   rel="tooltip" title="Avaliar consulta"
                                                   data-original-title="Avaliar consulta"
                                                   data-target="#modal-form" data-toggle="modal" data-whatever="@mdo"
                                                   onclick="setModal({{ $consulta->id }})">
                                                    Avaliar Paciente
                                                </a>
                                                ...
                                                @if ($consulta->status!="Finalizada")
                                                    @if (!$consulta->id_usuario_cancelou )
                                                            @if ($consulta->remota)
                                                                <a href="{{ $consulta->linkmeet }}"
                                                                   title="link teleatendimento" class="btn btn-info button-small-table"
                                                                   data-original-title="Link consulta" target="_blank"
                                                                   rel="noopener noreferrer"> Iniciar TeleAtendimento </a>
                                                            @endif
                                                        <br/>
                                                            <a rel="tooltip" title="Teleatendimento"
                                                               class="btn btn-warning button-small-table"
                                                               data-original-title="Edit" href="#" id="btnCanelarConsulta"
                                                               data-target="#modal-form-cancelar-consulta"
                                                               data-toggle="modal"
                                                               onclick="setModalCancelarConsulta({{ $consulta->id }}, {{ $helper::verificarPrazoCancelamentoGratuito($consulta->horario_agendado) }})">
                                                                Cancelar
                                                            </a>

                                                    @else
                                                        <button type="button"
                                                                class="btn btn-warning button-small-table consulta-cancelada">
                                                            Consulta cancelada
                                                        </button>
                                                    @endif
                                                @endif
    ...
    @component('layouts.modal_form', ["title" => "Como foi a sua consulta?", "route" => route('especialista.avaliacao.store'), "textButton" => "Salvar"])
        ...
        <div class="form-group">
            <textarea id="comentario-especialista" name="comentario_especialista" rows="2" cols="50" maxlength="200"
                      placeholder="Deixe algum comentário para o especialista (opcional)..."></textarea>
        </div>
