...
title="Por favor selecionar ..." required style="border-color: white">
   @foreach($especialidades as $iten)
     <option style="color: #2d3748"
       value="{{old('especialidade_id', $iten->id)}}"
        @if($iten->id == $entidade->especialidade_id)
           <?php    echo 'selected'; ?>
        @endif
        > {{$iten->descricao}}</option>
    @endforeach
   </select>
  </div>
 </div>
    ...
    <div class="col-md-5 px-8">
     <div class="form-group">
       ...
       <input style="border-color: #C0C0C0" type="number"  step=".01"
        min="0" class="form-control" name="valor" required value="{{$entidade->valor}}" maxlength="150">
    ...
    <a href="{{route('especialidadeclinica.list',$clinica->id)}}" class="btn btn-primary"><i class="fa fa-reply"></i><span> Voltar</span></a>
